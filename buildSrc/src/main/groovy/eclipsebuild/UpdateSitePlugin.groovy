...
class UpdateSitePlugin implements Plugin<Project> {
    ...
    static class Extension {
        ...
        def signing(Closure newSigning) {
            this.signing = newSigning
        }
    }
    ...
    static void addTaskSignBundles(Project project) {
        project.task(SIGN_BUNDLES_TASK_NAME, dependsOn: NORMALIZE_BUNDLES_TASK_NAME) {
            ...
            doLast { project.updateSite.signing(new SigningFolders(new File(project.buildDir, UNSIGNED_BUNDLES_DIR_NAME), new File(project.buildDir, SIGNED_BUNDLES_DIR_NAME))) }
