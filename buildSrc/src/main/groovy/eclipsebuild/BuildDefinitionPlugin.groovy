...
class BuildDefinitionPlugin implements Plugin<Project> {
    ...
    static void downloadEclipseSdk(Project project, Config config) {
        ...
        def directoryLock = new DirectoryLock(config.eclipseSdkDir)
        try {
            directoryLock.lock()
    ...
    static void addTaskAssembleTargetPlatform(Project project, Config config) {
        project.task(TASK_NAME_ASSEMBLE_TARGET_PLATFORM, dependsOn: [
            ...
            project.afterEvaluate { outputs.dir config.targetNonMavenizedPlatformDir }
    ...
    static void assembleTargetPlatform(Project project, Config config) {
        ...
        def lock = new DirectoryLock(config.targetNonMavenizedPlatformDir)
        try {
            lock.lock()
    ...
    static void assembleTargetPlatformUnprotected(Project project, Config config) {
        ...
        if (config.targetNonMavenizedPlatformDir.exists()) {
            project.logger.info("Delete mavenized platform directory '${config.targetNonMavenizedPlatformDir}'")
            config.targetNonMavenizedPlatformDir.deleteDir()
        }
        ...
        project.logger.info("Assemble org.eclipse.sdk.ide product in '${config.targetNonMavenizedPlatformDir.absolutePath}'.\n    Update sites: '${config.activeTargetPlatform.updateSites.join(' ')}'")
        project.exec {
            ...
            commandLine(config.eclipseSdkExe.path,
                    ...
                    '-repository', config.activeTargetPlatform.updateSites.join(','),
                    '-installIU', 'org.eclipse.sdk.ide/' + config.activeTargetPlatform.sdkVersion,
                    '-tag', '1-Initial-State',
                    '-destination', config.targetNonMavenizedPlatformDir.path,
                    '-profile', 'SDKProfile',
                    '-bundlepool', config.targetNonMavenizedPlatformDir.path,
                    '-p2.os', Constants.os,
                    '-p2.ws', Constants.ws,
        ...
        project.exec {
            ...
            commandLine(config.eclipseSdkExe.path,
                    '-application', 'org.eclipse.equinox.p2.director',
                    '-repository', config.activeTargetPlatform.updateSites.join(','),
                    '-installIU', config.activeTargetPlatform.features.join(','),
    ...
    static void addTaskInstallTargetPlatform(Project project, Config config) {
        project.task(TASK_NAME_INSTALL_TARGET_PLATFORM, dependsOn: TASK_NAME_ASSEMBLE_TARGET_PLATFORM) {
            ...
            project.afterEvaluate { inputs.dir config.targetNonMavenizedPlatformDir }
    ...
    static void installTargetPlatform(Project project, Config config) {
        ...
        // install bundles
        project.logger.info("Convert Eclipse target platform '${config.targetNonMavenizedPlatformDir}' to Maven repository '${config.mavenizedTargetPlatformDir}'")
        def deployer = new BundleMavenDeployer(project.ant, Constants.mavenEclipsePluginGroupName, project.logger)
        deployer.deploy(config.targetNonMavenizedPlatformDir, config.mavenizedTargetPlatformDir)
    }
