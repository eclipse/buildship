...
public class BaseConfigurator implements ProjectConfigurator {
    ...
    private Map<File, Pair<EclipseProject, Project>> locationToProject;
    ...
    public void init(InitializationContext context, IProgressMonitor monitor) {
        ...
        try {
            Collection<ExtendedEclipseModel> rootModels = gradleBuild.withConnection(connection -> {
                this.gradleVersion = GradleVersion.version(connection.getModel(BuildEnvironment.class).getGradle().getGradleVersion());
                Map<String, ExtendedEclipseModel> model = ExtendedEclipseModelUtils.queryModels(connection);

                return  model.values();
            }, monitor);
            Map<File, ? extends Project> projectModels = rootModels.stream()
                    .flatMap(m -> m.getProjects().stream())
                    .collect(Collectors.toMap(p -> p.getLocation(), p -> p));
            this.locationToProject = rootModels.stream()
                .flatMap(p -> HierarchicalElementUtils.getAll(p.getEclipseProject()).stream())
                .collect(Collectors.toMap(p -> p.getProjectDirectory(), p -> new Pair<>(p, projectModels.get(p.getProjectDirectory()))));
        } catch (Exception e) {
    ...
    private void configure(ProjectContext context, IProject project, IProgressMonitor monitor) throws CoreException {
        ...
        Pair<EclipseProject,Project> model2 = lookupEclipseModel2(project);
        progress.worked(1);
        ...
        if (isJavaProject(model)) {
            synchronizeJavaProject(context, model2, project, persistentModel, progress);
        } else {
    ...
    private void synchronizeJavaProject(final ProjectContext context, final Pair<EclipseProject,Project> model2, final IProject project, final PersistentModelBuilder persistentModel, SubMonitor progress) throws CoreException {
        JavaCore.run(new IWorkspaceRunnable() {
            @Override
            public void run(IProgressMonitor monitor) throws CoreException {
                SubMonitor progress = SubMonitor.convert(monitor);
                synchronizeJavaProjectInTransaction(context, model2, project, persistentModel, progress);
            }
        }, progress.newChild(1));
    }

    private void synchronizeJavaProjectInTransaction(final ProjectContext context, final Pair<EclipseProject,Project> model2, final IProject project, PersistentModelBuilder persistentModel, SubMonitor progress) throws JavaModelException, CoreException {
        ...
        OutputLocationUpdater.update(context, javaProject, model2.getFirst(), progress.newChild(1));
        SourceFolderUpdater.update(javaProject, ImmutableList.copyOf(model2.getFirst().getSourceDirectories()), progress.newChild(1));
        LibraryFilter.update(javaProject, model2.getFirst(), progress.newChild(1));
        ClasspathContainerUpdater.update(javaProject, model2.getFirst(), progress.newChild(1));
        JavaSourceSettingsUpdater.update(javaProject, model2, progress.newChild(1));
        GradleClasspathContainerUpdater.updateFromModel(javaProject, model2.getFirst(), this.locationToProject.values(), persistentModel, progress.newChild(1), context);
        persistentModel.hasAutoBuildTasks(model2.getFirst().hasAutoBuildTasks());
    }
    ...
    private EclipseProject lookupEclipseModel(IProject project) {
        ...
        return this.locationToProject.get(path.toFile()).getFirst();
    }

    private Pair<EclipseProject,Project> lookupEclipseModel2(IProject project) {
        IPath path = project.getLocation();
        if (path == null) {
            return null;
        }
