...
class BuildConfigurationTest extends ProjectSynchronizationSpecification {
    ...
    def "new build configuration can inherit workspace settings"(GradleDistribution distribution, boolean buildScansEnabled, boolean offlineMode, boolean autoRefresh) {
        setup: "this "
        ...
        when:
        configurationManager.saveWorkspaceConfiguration(new WorkspaceConfiguration(distribution, workspaceGradleUserHome, offlineMode, buildScansEnabled, autoRefresh))
        ...
        where:
        distribution                                                                 | offlineMode  | buildScansEnabled | autoRefresh
    ...
    def "new build configuration can override workspace settings"(GradleDistribution distribution, boolean buildScansEnabled, boolean offlineMode, boolean autoRefresh) {
        setup:
        File projectDir = dir('project-dir')
        File projectGradleUserHome = dir('gradle-user-home').canonicalFile

        when:
        BuildConfiguration configuration = createOverridingBuildConfiguration(projectDir, distribution, buildScansEnabled, offlineMode, autoRefresh, projectGradleUserHome)
...
connection.gradle.distribution=INVALID_GRADLE_DISTRO"""
    ...
    def "load build configuration respecting workspaces settings"(GradleDistribution distribution, boolean buildScansEnabled, boolean offlineMode, boolean autoRefresh) {
        setup:
        File projectDir = dir('project-dir')
    ...
    def "load build configuration overriding workspace settings"(GradleDistribution distribution, boolean buildScansEnabled, boolean offlineMode, boolean autoRefresh) {
        setup:
        File projectDir = dir('project-dir')
        WorkspaceConfiguration originalWsConfig = configurationManager.loadWorkspaceConfiguration()
        File projectGradleUserHome = dir('gradle-user-home').canonicalFile
        BuildConfiguration buildConfig = createOverridingBuildConfiguration(projectDir, distribution, buildScansEnabled, offlineMode, autoRefresh, projectGradleUserHome)

        when:
        configurationManager.saveBuildConfiguration(buildConfig)
        configurationManager.saveWorkspaceConfiguration(new WorkspaceConfiguration(GradleDistribution.fromBuild(), null, !buildScansEnabled, !offlineMode, !autoRefresh))
        buildConfig = configurationManager.loadBuildConfiguration(projectDir)
