...
public final class GradleWorkspaceCompositePreferencePage extends PropertyPage {
    ...
    protected Control createContents(Composite parent) {
    	this.gradleWorkspaceCompositeSettingsComposite = builderFactory.newComposite(parent).control();
    	gradleWorkspaceCompositeSettingsComposite.setLayout(LayoutUtils.newGridLayout(2));

        // composite name container
        Composite workspaceCompositeNameComposite = builderFactory.newComposite(gradleWorkspaceCompositeSettingsComposite).control();
        ...
        builderFactory.newLabel(workspaceCompositeNameComposite).alignLeft().text(WorkspaceCompositeWizardMessages.Label_CompositeName).control();

        // composite name text field
        this.workspaceCompositeNameText = builderFactory.newText(workspaceCompositeNameComposite).alignFillHorizontal().control();
