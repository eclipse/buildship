...
public class ExternalProjectDialogSelectionListener extends SelectionAdapter {
	...
	public ExternalProjectDialogSelectionListener(Shell shell, TreeViewer treeViewer, String entity) {
        ...
        ColumnViewerToolTipSupport.enableFor(projectTreeViewer);
        externalProjectPaths = new HashMap<String, String>();
	}
	...
	private void addExternalProjectToProjectTree(String selectedDirectory) {
		...
		if (gradleSettingsFile.isFile()) {	
			try {
            	...
            	if (!externalProjectPaths.containsKey(gradleSettingsFile.getParentFile().getPath())) {
                	externalProjectPaths.put(gradleSettingsFile.getParentFile().getPath(), projectName);
            	}
			} catch (IOException e) {
				e.printStackTrace();
			}
