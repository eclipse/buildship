...
public final class GradleOptionsWizardPage extends AbstractWizardPage {
    ...
    public GradleOptionsWizardPage(ProjectImportConfiguration configuration, PublishedGradleVersions publishedGradleVersions) {
        super("GradleOptions", ProjectImportMessages.Title_GradleOptionsWizardPage, ProjectImportMessages.InfoMessage_GradleOptionsWizardPageDefault, //$NON-NLS-1$
                configuration, ImmutableList.<Property<?>> of(configuration.getGradleDistribution(), configuration.getJavaHome(), configuration.getGradleUserHome(), configuration.getJvmArguments()));
    ...
    private void createGradleDistributionContent(Composite root) {
        ...
        this.useGradleWrapperOption = uiBuilderFactory.newRadio(root).alignLeft().text(CoreMessages.GradleOptions_Label_GradleWrapper).control();

        uiBuilderFactory.span(root);
        uiBuilderFactory.span(root);

        // second line: local installation directory
        this.useLocalInstallationDirOption = uiBuilderFactory.newRadio(root).alignLeft().text(CoreMessages.GradleOptions_Label_LocalInstallationDirectory).control();
        ...
        this.useRemoteDistributionUriOption = uiBuilderFactory.newRadio(root).alignLeft().text(CoreMessages.GradleOptions_Label_RemoteDistributionUri).control();
        ...
        this.useGradleVersionOption = uiBuilderFactory.newRadio(root).alignLeft().text(CoreMessages.GradleOptions_Label_SpecificGradleVersion).control();
