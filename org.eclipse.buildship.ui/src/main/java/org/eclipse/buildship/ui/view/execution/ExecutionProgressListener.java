...
/**
 * Listens to {@link org.gradle.tooling.events.ProgressEvent} instances that are
 * sent by the Tooling API while a build is executed. Each incoming event is
 * added to the execution tree as an {@link OperationItem} instance.
 */
public final class ExecutionProgressListener implements org.gradle.tooling.events.ProgressListener {
    ...
    public void statusChanged(ProgressEvent progressEvent) {
        // do not process the event if the operation is technical/artificial,
        // i.e. the event
        ...
        // create the job to update the duration of running operations as late
        // as possible
        // to make sure it is only created if really needed
        initDurationJobIfNeeded();

        // create a new operation item if the event is a start event, otherwise
        // update the item
        ...
        // attach to (first non-excluded) parent, if this is a new operation (in
        // case of StartEvent)
        ...
        // ensure that if it is a newly added node it is made visible (in case
        // of StartEvent)
    ...
    private void initDurationJobIfNeeded() {
        if (this.updateDurationJob == null) {
            this.updateDurationJob = new UpdateDurationJob(UPDATE_DURATION_JOB_INTERVAL_IN_MS,
                    this.operationItemRenderer);
    ...
    private boolean isExcluded(OperationDescriptor descriptor) {
        // ignore the 'artificial' events issued for the root test event and for
        // each forked test
        // process event
        if (descriptor instanceof JvmTestOperationDescriptor) {
            JvmTestOperationDescriptor jvmTestOperationDescriptor = (JvmTestOperationDescriptor) descriptor;
            return jvmTestOperationDescriptor.getSuiteName() != null
                    && jvmTestOperationDescriptor.getClassName() == null;
        } else {
            return false;
        }
    }
