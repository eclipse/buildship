...
/**
 * Shows the first page of the {@link ExampleTaskTypeWizard}, where the user sets the 
 * task type and the name for the function.
 */
public class CreateTaskTypeWizardPageOne extends WizardPage{
    ...
    /**
     * Checks, if the finish and next buttons could be available and 
     * sets the {@link TaskType} to the second page.
     */
    private void updatePageComplete() {
        setPageComplete(false);
        if(getTaskName().length() > 0 && getSelectedTaskType()
                != null){
     ...
     * @param currentTaskType {@link TaskType}
     */
