...
/**
 * Utility class for {@link NodeSelection}.
 */
public final class TaskNodeSelectionUtils {
    ...
    /**
     * Returns a {@link GradleRunConfigurationAttributes} instance for a target
     * {@link NodeSelection} if the selection can be translated to it.
     *
     * @param selectionHistory the target {@link NodeSelection}
     * @return The configured {@link NodeSelection} of {@link Optional#absent()} if the conversion
     *         can't be completed.
     */
    public static Optional<GradleRunConfigurationAttributes> getRunConfigurationAttributes(NodeSelection selectionHistory) {
        Preconditions.checkNotNull(selectionHistory, "");
        ...
        if (requestAttributes.isPresent()) {
            // determine the string representation of the tasks to run, relative to the project
            // directory (we currently work with Strings since Launchable is only available as of
            // 1.12)
            ...
            return Optional.of(GradleRunConfigurationAttributes
                    .with(tasks, projectDirectoryExpression, gradleDistribution, gradleUserHome, javaHome, jvmArguments, arguments, true));
        } else {
    ...
    private static String getProjectDirectoryExpression(NodeSelection selection) {
        if (TaskViewActionStateRules.taskScopedTaskExecutionActionsEnabledFor(selection)) {
            // task selectors need to be run from the direct parent directory, project tasks can be
            // run
            ...
            // the default tasks of a project must be run from the direct parent directory
            // (obviously)
