...
public final class DialogUserNotification implements UserNotification {
    ...
    @Override
    public void errorOccurred(final String message, final String details, final Throwable throwable) {
        Display.getDefault().syncExec(new Runnable() {

            @Override
            public void run() {
                Shell shell = Display.getDefault().getActiveShell();
                ExceptionDetailsDialog dialog = new ExceptionDetailsDialog(shell, message, details, throwable);
                dialog.open();
            }
        });
    }
